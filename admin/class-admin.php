...
class Help_Manager_Admin {
	...
	public function list_pages_add_handle( $html ) {
		if( $this->current_user_is_editor() ) {
			$html = preg_replace( '#<li [^>]+>#', '$0<span><img class="sort-handle" src="' . plugin_dir_url( __FILE__ ) . 'assets/img/handle.png">', $html );
			$html = preg_replace( '#</a>#', '$0</span>', $html );
		} else {
			$html = preg_replace( '#<li [^>]+>#', '$0<span><img class="wphm-document-icon" src="' . plugin_dir_url( __FILE__ ) . 'assets/img/document.png">', $html );
			$html = preg_replace( '#</a>#', '$0</span>', $html );
		}
		return $html;
	}
	...
	public function list_pages_add_icon( $html ) {
		$html = preg_replace( '#<a [^>]+>#', '$0<img class="wphm-document-icon" src="' . plugin_dir_url( __FILE__ ) . 'assets/img/document.png"><span>', $html );
